# Server vs Client — Code Snippet Library (starter)

A copy of `apps/06-nextjs/app-router/code-snippet-library`, scaffolded as
...
What's already here from the app-router version: the `snippetsService`
(in-memory array), and the `/snippets` list and `/snippets/[id]` detail
page.
...
Work through the "Code Snippet Library" challenges in `challenges.md`, in
order. They mirror the Kiki's challenges:

1. **Filter snippets by language**
2. **Add a "new snippet" form**
3. **Add a snippets API route**

Follow `challenges.md` directly for these three — the steps there apply to
this package as-is. Come back here for the fourth one.

## 4. Back the library with Postgres — with Prisma instead of `postgres`
...
2. **Write the `Snippet` model.** Open `prisma/schema.prisma` and translate
   the `snippets` table from `challenges.md` into a Prisma model — an `id`,
   `title`, `language`, `description`, and `code` field.

3. **Migrate.** From this folder:

   ```bash
   pnpm exec prisma migrate dev --name add_snippet
   ```
...
4. **Seed it.** Create `prisma/seed.ts` (a small script that connects with
   ...
   `prisma.snippet.createMany()` — you can reuse the three starter snippets
   already in `lib/services/snippetsService.ts` as the seed data). Wire it
   up in `prisma.config.ts`'s `migrations.seed` (already pointed at
   `tsx prisma/seed.ts`), then run:

   ```bash
   pnpm run db:seed
   ```

5. **Rewrite `snippetsService`.** Replace the in-memory array in
   `lib/services/snippetsService.ts` with Prisma queries against
   `import { prisma } from '@/lib/prisma'` — `prisma.snippet.findMany()`,
   `prisma.snippet.findUnique()`, `prisma.snippet.create()` — keeping the
   same function names and signatures (`getAllSnippets`, `getSnippetById`,
   `createSnippet`) so the rest of the app doesn't need to change.

6. **Confirm.** The list, detail pages, filter, form, and API routes should
   all still work against the database, and snippets should persist across
   a restart.
